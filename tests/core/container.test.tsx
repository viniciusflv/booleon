...
describe('Container', () => {
  describe('Content', () => {
    test('content default', () => {
      const { getByTestId } = render(<View.div data-testid='content' content />);
  ...
  describe('Z-Index', () => {
    test('z_666', () => {
      const { getByTestId } = render(<View.div data-testid='z_666' z_666 />);
      expect(getByTestId('z_666')).toHaveStyle('z-index: 666;');
    });
  
    test('z_max', () => {
      const MAX_SAFE_INTEGER = Math.pow(2, 31) - 1;
      const { getByTestId } = render(<View.div data-testid='z_max' z_max />);
      expect(getByTestId('z_max')).toHaveStyle(`z-index: ${MAX_SAFE_INTEGER};`);
    });
  
    test('z_neg', () => {
      const { getByTestId } = render(<View.div data-testid='z_neg' z_neg />);
      expect(getByTestId('z_neg')).toHaveStyle('z-index: -1;');
    });
  
    test('z_auto', () => {
      const { getByTestId } = render(<View.div data-testid='z_auto' z_auto />);
  ...
  describe('Placement', () => {
    describe('Top', () => {
      test('top_666', () => {
        const { getByTestId } = render(<View.div data-testid='top_666' top_666 />);
        expect(getByTestId('top_666')).toHaveStyle('top: 6.66rem;');
      });
  
      test('top_neg_666', () => {
        const { getByTestId } = render(<View.div data-testid='top_neg_666' top_neg_666 />);
        expect(getByTestId('top_neg_666')).toHaveStyle('top: -6.66rem;');
      });
  
      test('top', () => {
        const { getByTestId } = render(<View.div data-testid='top' top />);
        expect(getByTestId('top')).toHaveStyle('top: 0;');
      });
  
      test('top_neg', () => {
        const { getByTestId } = render(<View.div data-testid='top_neg' top_neg />);
    ...
    describe('Bottom', () => {
      test('bottom_666', () => {
        const { getByTestId } = render(<View.div data-testid='bottom_666' bottom_666 />);
        expect(getByTestId('bottom_666')).toHaveStyle('bottom: 6.66rem;');
      });
  
      test('bottom_neg_666', () => {
        const { getByTestId } = render(<View.div data-testid='bottom_neg_666' bottom_neg_666 />);
        expect(getByTestId('bottom_neg_666')).toHaveStyle('bottom: -6.66rem;');
      });
  
      test('bottom', () => {
        const { getByTestId } = render(<View.div data-testid='bottom' bottom />);
        expect(getByTestId('bottom')).toHaveStyle('bottom: 0;');
      });
  
      test('bottom_neg', () => {
        const { getByTestId } = render(<View.div data-testid='bottom_neg' bottom_neg />);
    ...
    describe('Left', () => {
      test('left_666', () => {
        const { getByTestId } = render(<View.div data-testid='left_666' left_666 />);
        expect(getByTestId('left_666')).toHaveStyle('left: 6.66rem;');
      });
  
      test('left_neg_666', () => {
        const { getByTestId } = render(<View.div data-testid='left_neg_666' left_neg_666 />);
        expect(getByTestId('left_neg_666')).toHaveStyle('left: -6.66rem;');
      });
  
      test('left', () => {
        const { getByTestId } = render(<View.div data-testid='left' left />);
        expect(getByTestId('left')).toHaveStyle('left: 0;');
      });
  
      test('left_neg', () => {
        const { getByTestId } = render(<View.div data-testid='left_neg' left_neg />);
    ...
    describe('Right', () => {
      test('right_666', () => {
        const { getByTestId } = render(<View.div data-testid='right_666' right_666 />);
        expect(getByTestId('right_666')).toHaveStyle('right: 6.66rem;');
      });
  
      test('right_neg_666', () => {
        const { getByTestId } = render(<View.div data-testid='right_neg_666' right_neg_666 />);
        expect(getByTestId('right_neg_666')).toHaveStyle('right: -6.66rem;');
      });
  
      test('right', () => {
        const { getByTestId } = render(<View.div data-testid='right' right />);
        expect(getByTestId('right')).toHaveStyle('right: 0;');
      });
  
      test('right_neg', () => {
        const { getByTestId } = render(<View.div data-testid='right_neg' right_neg />);
    ...
    describe('Inset', () => {
      test('inset', () => {
        const { getByTestId } = render(<View.div data-testid='inset' inset />);
      ...
      test('inset_x', () => {
        const { getByTestId } = render(<View.div data-testid='inset_x' inset_x />);
        expect(getByTestId('inset_x')).toHaveStyle('left: 0;');
        expect(getByTestId('inset_x')).toHaveStyle('right: 0;');
      });

      test('inset_y', () => {
        const { getByTestId } = render(<View.div data-testid='inset_y' inset_y />);
  ...
  describe('Position', () => {
    test('fixed', () => {
      const { getByTestId } = render(<View.div data-testid='fixed' fixed />);
      expect(getByTestId('fixed')).toHaveStyle('position: fixed;');
    });

    test('absolute', () => {
      const { getByTestId } = render(<View.div data-testid='absolute' absolute />);
      expect(getByTestId('absolute')).toHaveStyle('position: absolute;');
    });

    test('relative', () => {
      const { getByTestId } = render(<View.div data-testid='relative' relative />);
      expect(getByTestId('relative')).toHaveStyle('position: relative;');
    });

    test('sticky', () => {
      const { getByTestId } = render(<View.div data-testid='sticky' sticky />);
  ...
  describe('Scroll', () => {
    describe('Auto', () => {
      test('sc_auto', () => {
        const { getByTestId } = render(<View.div data-testid='sc_auto' sc_auto />);
        expect(getByTestId('sc_auto')).toHaveStyle('overflow: auto;');
      });

      test('scy_auto', () => {
        const { getByTestId } = render(<View.div data-testid='scy_auto' scy_auto />);
        expect(getByTestId('scy_auto')).toHaveStyle('overflow-y: auto;');
      });

      test('scx_auto', () => {
        const { getByTestId } = render(<View.div data-testid='scx_auto' scx_auto />);
    ...
    describe('Hidden', () => {
      test('sc_hidden', () => {
        const { getByTestId } = render(<View.div data-testid='sc_hidden' sc_hidden />);
        expect(getByTestId('sc_hidden')).toHaveStyle('overflow: hidden;');
      });

      test('scy_hidden', () => {
        const { getByTestId } = render(<View.div data-testid='scy_hidden' scy_hidden />);
        expect(getByTestId('scy_hidden')).toHaveStyle('overflow-y: hidden;');
      });

      test('scx_hidden', () => {
        const { getByTestId } = render(<View.div data-testid='scx_hidden' scx_hidden />);
    ...
    describe('Visible', () => {
      test('sc_visible', () => {
        const { getByTestId } = render(<View.div data-testid='sc_visible' sc_visible />);
        expect(getByTestId('sc_visible')).toHaveStyle('overflow: visible;');
      });

      test('scy_visible', () => {
        const { getByTestId } = render(<View.div data-testid='scy_visible' scy_visible />);
        expect(getByTestId('scy_visible')).toHaveStyle('overflow-y: visible;');
      });

      test('scx_visible', () => {
        const { getByTestId } = render(<View.div data-testid='scx_visible' scx_visible />);
  ...
  describe('Visibility', () => {
    test('hidden', () => {
      const { getByTestId } = render(<View.div data-testid='hidden' hidden />);
      expect(getByTestId('hidden')).toHaveStyle('display: none;');
    });

    test('visible', () => {
      const { getByTestId } = render(<View.div data-testid='visible' visible />);
      expect(getByTestId('visible')).toHaveStyle('visibility: visible;');
    });

    test('invisible', () => {
      const { getByTestId } = render(<View.div data-testid='invisible' invisible />);
      expect(getByTestId('invisible')).toHaveStyle('visibility: hidden;');
    });

    test('noappearance', () => {
      const { getByTestId } = render(<View.div data-testid='noappearance' noappearance />);
  ...
  describe('Fit', () => {
    test('fit_contain', () => {
      const { getByTestId } = render(<View.div data-testid='fit_contain' fit_contain />);
      expect(getByTestId('fit_contain')).toHaveStyle('object-fit: contain;');
    });

    test('fit_cover', () => {
      const { getByTestId } = render(<View.div data-testid='fit_cover' fit_cover />);
      expect(getByTestId('fit_cover')).toHaveStyle('object-fit: cover;');
    });

    test('fit_fill', () => {
      const { getByTestId } = render(<View.div data-testid='fit_fill' fit_fill />);
      expect(getByTestId('fit_fill')).toHaveStyle('object-fit: fill;');
    });

    test('fit_none', () => {
      const { getByTestId } = render(<View.div data-testid='fit_none' fit_none />);
  ...
  describe('Resize', () => {
    test('rz_none', () => {
      const { getByTestId } = render(<View.div data-testid='rz_none' rz_none />);
      expect(getByTestId('rz_none')).toHaveStyle('resize: none;');
    });

    test('rz', () => {
      const { getByTestId } = render(<View.div data-testid='rz' rz />);
      expect(getByTestId('rz')).toHaveStyle('resize: both;');
    });

    test('rzy', () => {
      const { getByTestId } = render(<View.div data-testid='rzy' rzy />);
      expect(getByTestId('rzy')).toHaveStyle('resize: vertical;');
    });

    test('rzx', () => {
      const { getByTestId } = render(<View.div data-testid='rzx' rzx />);
