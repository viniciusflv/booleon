...
const handleGradient = (key: string, match: string) => {
  ...
  case 'gy_':
    return linearGradient('to top', `#${color}`, 'transparent', `#${color}`);
  case 'gt_':
    return linearGradient('to top', 'transparent', `#${color}`);
  case 'gb_':
    return linearGradient('to bottom', 'transparent', `#${color}`);
  case 'gl_':
    return linearGradient('to left', 'transparent', `#${color}`);
  case 'gr_':
    return linearGradient('to right', 'transparent', `#${color}`);
  default:
    return '';
  }
};
...
const keyStartsWith = (match: string, props: BackgroundProps) => {
  const a = Object.keys(props)
    .map((key: string) => key.startsWith(match))
    .includes(true);
  return a;
};
...
export const BackgroundStyle = styled.div<BackgroundProps>`
  ...
  ${(props) =>
    Object.keys(props).reduce(
      (acc, key) =>
        key.startsWith('op_') ? (acc += handleOpacity(key, 'op')) : acc,
      '',
    )}
  ${(props) => keyStartsWith('g', props) && gradientCss}
  ${(props) => keyStartsWith('bg', props) && backgroundCss}
`;
