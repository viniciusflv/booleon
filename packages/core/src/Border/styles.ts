...
const handleBorderColor = (key: string) => {
  ...
  case 'bxc': return `border-right-color: #${color};border-left-color: #${color};`;
  case 'byc': return `border-top-color: #${color};border-bottom-color: #${color};`;
...
const borderColor = (props: BorderProps) =>
  Object.keys(props).reduce((acc: string, key: string) => {
    if (key.startsWith('bc') || key.match(/b[a-z]c/)) return handleBorderColor(key);
    return acc;
  }, '');
...
export const BorderStyle = styled.div<BorderProps>`
  ...
  ${({ bx_thick }) =>
    bx_thick && 'border-right-width: .5rem;order-left-width: .5rem;'}
  ${({ bx_thin }) =>
    bx_thin && 'border-right-width: .05rem;order-left-width: .05rem;'}
  ...
  ${({ by_thick }) =>
    by_thick && 'border-top-width: .5rem;order-bottom-width: .5rem;'}
  ${({ by_thin }) =>
    by_thin && 'border-top-width: .05rem;order-bottom-width: .05rem;'}
  ...
  ${({ bt_rounded }) =>
    bt_rounded && 'border-top-right-radius: 2rem;border-top-left-radius: 2rem;'}
  ${({ bt_edge }) =>
    bt_edge && 'border-top-right-radius: .5rem;border-top-left-radius: .5rem;'}
