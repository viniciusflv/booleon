...
export const border = [
  ...
  [
    ['bs_' as `bs_${BorderStyle}`, `(${LOWER_CASE})`],
    (value: string) => `border-style:${value};`,
  ],
  [
    ['bts_' as `bts_${BorderStyle}`, `(${LOWER_CASE})`],
    (value: string) => `border-top-style:${value};`,
  ],
  [
    ['bbs_' as `bbs_${BorderStyle}`, `(${LOWER_CASE})`],
    (value: string) => `border-bottom-style:${value};`,
  ],
  [
    ['bls_' as `bls_${BorderStyle}`, `(${LOWER_CASE})`],
    (value: string) => `border-left-style:${value};`,
  ],
  [
    ['brs_' as `brs_${BorderStyle}`, `(${LOWER_CASE})`],
    (value: string) => `border-right-style:${value};`,
  ],
  [
    ['bxs_' as `bxs_${BorderStyle}`, `(${LOWER_CASE})`],
    (value: string) =>
      `border-left-style:${value};border-right-style:${value};`,
  ],
  [
    ['bys_' as `bys_${BorderStyle}`, `(${LOWER_CASE})`],
    (value: string) =>
